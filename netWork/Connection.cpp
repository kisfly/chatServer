...
{
    LOG_INFO("appendwithsep1：%d",data->size());
    outputbuffer_.appendwithsep(data->data(),data->size());    // 把需要发送的数据保存到Connection的发送缓冲区中。
    LOG_INFO("appendwithsep2：%d",outputbuffer_.size());
